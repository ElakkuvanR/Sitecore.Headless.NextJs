...
if($SqlAdminUser -and $SqlAdminPassword) {
    & sqlcmd -I -i (Join-Path $ResourcesDirectory 'SetSitecoreAdminPassword.sql') -d "$DatabasePrefixName.Sitecore.Core" -S "$SqlServer" -U "$SqlAdminUser" -P "$SqlAdminPassword"
} else {
    & sqlcmd -I -i (Join-Path $ResourcesDirectory 'SetSitecoreAdminPassword.sql') -d "$DatabasePrefixName.Sitecore.Core" -S "$SqlServer"
}
